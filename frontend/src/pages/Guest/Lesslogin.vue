...
function submit() {
  axiosClient.get('/sanctum/csrf-cookie').then(response => {
    axiosClient.post('/lessor/login', data.value)
    .then(response => {
...
<template>
    ...
    <GuestLayout>
        <h2 class="mt-10 text-center text-2xl/9 font-bold tracking-tight text-gray-900">Welcome back lessor</h2>
    ...
    <div class="mt-4 sm:mx-auto sm:w-full sm:max-w-sm">
      ...
      <p class="mt-10 text-center text-sm/6 text-gray-500">
        Login as a client
        {{ ' ' }}
      <router-link :to="{name: 'Login'}" class="font-semibold text-indigo-600 hover:text-indigo-500">Click here</router-link>
      </p>
